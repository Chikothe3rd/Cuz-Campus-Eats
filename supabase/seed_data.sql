...
-- Convert to a pure SQL/PLpgSQL script compatible with Supabase SQL editor

do $$
declare
  buyer_email text := 'buyer@campuseats.test';
  vendor_email text := 'vendor@campuseats.test';
  runner_email text := 'runner@campuseats.test';

  buyer_id  uuid := '11111111-2222-3333-4444-555555555555';
  vendor_id uuid := '66666666-7777-8888-9999-aaaaaaaaaaaa';
  runner_id uuid := 'bbbbbbbb-cccc-dddd-eeee-ffffffffffff';
...
begin
  ...
  -- Insert auth users (email confirmed)
  ...
  -- Update profiles with extras (created by trigger on auth.users)
  update public.profiles set name = 'Campus Buyer', phone = '+260970000001', campus_name = 'Cavendish Main' where id = buyer_id;
  update public.profiles set name = 'Fresh Bites Owner', phone = '+260970000002', campus_name = 'Cavendish Main' where id = vendor_id;
  update public.profiles set name = 'Speedy Runner', phone = '+260970000003', campus_name = 'Cavendish Main' where id = runner_id;

  -- Insert vendor and capture id
  ...
  -- Insert menu items
  insert into public.menu_items (vendor_id, name, description, price, image_url, category, preparation_time, is_available) values
